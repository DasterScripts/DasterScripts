--[[
    ════════════════════════════════════════════════════════════════════
    ⛏ DASTER SCRIPTS — DRILL TO THE CORE — PERFECT v4.0 ⛏
    ════════════════════════════════════════════════════════════════════
    Author: Daster | Оптимизированная • Стабильная • Максимум функций
    ════════════════════════════════════════════════════════════════════
    
    ⚠ ЧЕСТНОЕ ПРЕДУПРЕЖДЕНИЕ:
    Серверные значения (монеты, инвентарь, глубина) НЕЛЬЗЯ изменить 
    клиентским скриптом. Реально работает только:
    - Auto-farm (руда летит к тебе)
    - ESP, Fly, Noclip, Speed, God
    - Remote-взлом (если сервер не защищён)
    
    Всё остальное — визуальный обман или лотерея.
    ════════════════════════════════════════════════════════════════════
]]
...
local MainFrame = U.Create("Frame", {
    Name = "MainFrame",
    Size = UDim2.new(0, 740, 0, 540),
    Position = UDim2.new(0.5, -370, 0.5, -270),
...
U.Create("TextLabel", {
    ...
    BackgroundTransparency = 1, Text = "⛏ Perfect Edition ⛏ v" .. CONFIG.Version,
...
MinBtn.MouseButton1Click:Connect(function()
    ...
    local target = minimized and UDim2.new(0, 740, 0, 55) or UDim2.new(0, 740, 0, 540)
    ...
    for _, target in ipairs(targets) do
        for _, obj in ipairs(target:GetDescendants()) do
            if obj:IsA("RemoteEvent") or obj:IsA("RemoteFunction") then
                ...
                if string.find(n, "upgrade") or string.find(n, "improve") or string.find(n, "enhance") then
...
Section(InfoPage, "⛏ Daster Drill PERFECT")
Label(InfoPage, "⚔ Версия: v" .. CONFIG.Version)
Label(InfoPage, "🎮 Игра: Бурите к ядру Земли")
Label(InfoPage, "👤 Автор: Daster")
Label(InfoPage, "")

Section(InfoPage, "⚠ ЧЕСТНОЕ ПРЕДУПРЕЖДЕНИЕ")
Label(InfoPage, "Серверные значения нельзя взломать.", CONFIG.Theme.Error)
Label(InfoPage, "Монеты/инвентарь/глубина — на сервере.", CONFIG.Theme.Error)
Label(InfoPage, "Реально работает:", CONFIG.Theme.Success)
Label(InfoPage, "✅ Auto-Mine (руда летит к тебе)")
Label(InfoPage, "✅ Auto-Pickup")
Label(InfoPage, "✅ ESP, Fly, Noclip, Speed, God")
Label(InfoPage, "✅ Auto-Sell (если remote не защищён)")
Label(InfoPage, "")
Label(InfoPage, "❌ Не работает (никогда):")
Label(InfoPage, "❌ Прямое начисление монет")
Label(InfoPage, "❌ Выдача Tools в инвентарь")
Label(InfoPage, "❌ Изменение глубины/уровня")
...
Section(GivePage, "⚠ ЧЕСТНО ПРО ВЫДАЧУ")
Label(GivePage, "Копирование Tools в Backpack работает ТОЛЬКО визуально.", CONFIG.Theme.Error)
Label(GivePage, "Сервер не видит предмет — использовать не получится.", CONFIG.Theme.Error)
Label(GivePage, "Реально выдастся ТОЛЬКО если найдём нужный Remote.", CONFIG.Theme.Success)

Section(GivePage, "🔍 Сканирование")
...
Button(GivePage, "🎁 ВЫДАТЬ ВСЕ Tools (визуально)", function()
    local count = 0
    for _, obj in ipairs(ReplicatedStorage:GetDescendants()) do
    ...
    U.Notify("Скопировано (визуально): " .. count, 3)
end)

Section(GivePage, "🎁 Выдать через Remote")
Label(GivePage, "Попробует через найденные remote (Give/Tool/Equip)")
TextBox(GivePage, "Название предмета", function(text)
    if not REMOTES.Scanned then ScanRemotes() end
    local sent = 0
    for _, remote in ipairs(REMOTES.Give) do
        pcall(function()
            if remote:IsA("RemoteEvent") then
                remote:FireServer(text)
                remote:FireServer("give", text)
                remote:FireServer({item = text, name = text})
            else
    ...
    U.Notify("Отправлено remote: " .. sent, 3)
end)

Section(GivePage, "⚔ Быстрые кнопки")
for _, item in ipairs({"Sword", "Меч", "Pickaxe", "Кирка", "Drill", "Бур", "Helmet", "Шлем", "Shield", "Щит"}) do
    Button(GivePage, "🎁 " .. item, function()
        ...
        if sent == 0 then
            U.Notify("Give remotes не найдены", 2, CONFIG.Theme.Error)
        else
            U.Notify("Отправлено: " .. sent, 2)
        end
    end)
end
...
Section(CoinPage, "⚠ ЧЕСТНО ПРО МОНЕТЫ")
Label(CoinPage, "Сервер не даст начислить монеты клиентом.", CONFIG.Theme.Error)
Label(CoinPage, "Работает ТОЛЬКО Auto-Sell (продажа руды).", CONFIG.Theme.Success)
...
Button(CoinPage, "💰 Продать ВСЁ сейчас", function()
    ...
    U.Notify("Продажа: " .. sent .. " remotes", 3)
end)

Section(CoinPage, "💎 Remote-атака (экспериментально)")
...
Button(CoinPage, "💰 +1000000 монет (попытка)", function()
    if not REMOTES.Scanned then ScanRemotes() end
    for _, remote in ipairs(REMOTES.Money) do
        pcall(function()
            if remote:IsA("RemoteEvent") then
...
Toggle(PlayerPage, "⚔ Kill Aura (25 studs)", function(state)
...
Button(RemPage, "📋 Все remotes в консоль", function()
    ...
    U.Notify("Всего: " .. #REMOTES.All .. " (см. F9)", 3)
end)
...
Button(SetPage, "🗑 Отключить ВСЁ", function()
    ...
    for _, conn in pairs({STATE.FlyConn, STATE.GodConn, STATE.InfJumpConn,
                          STATE.NoclipConn, STATE.DrillConn, STATE.KillAuraConn}) do
...
--============================================================
-- АВТО-СКРОЛЛ (обновляем CanvasSize)
--============================================================
task.spawn(function()
...
task.wait(0.5)
U.Notify("🔍 Remotes → Сканировать сначала!", 4, CONFIG.Theme.Info)

print("[DasterCore] ═══════════════════════════════════════")
print("[DasterCore] ⛏ v" .. CONFIG.Version .. " загружен!")
print("[DasterCore] Вкладок: " .. tostring(#TabScroll:GetChildren() - 1))
print("[DasterCore] ⚠ Серверные значения нельзя взломать")
print("[DasterCore] ✅ Работает: AutoMine, ESP, Fly, Noclip, Speed, God")
print("[DasterCore] ═══════════════════════════════════════")
